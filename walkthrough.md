# Fix: Ready Button Click Gagal — GuiService Inset Offset

## Masalah

Script `SummonHero_AutoFarm_Entries.lua` gagal meng-click tombol **ReadyButton** meskipun tombol terdeteksi dengan benar. Semua 18 metode click menghasilkan `EXEC/NOPROG` (executed tapi tidak ada progress).

## Root Cause: Koordinat Meleset 58px

Semua metode yang menggunakan **VirtualInputManager** dan **executor mouse functions** (`mousemoveabs`, `mouse1click`, dll.) bekerja dengan **screen coordinates** (koordinat layar absolut). Namun, properti `AbsolutePosition` di Roblox mengembalikan posisi **relatif terhadap viewport** — yaitu area di bawah **top bar** Roblox.

```
┌──────────────────────────────────┐
│         TOP BAR (58px)           │  ← Area ini tidak dihitung oleh AbsolutePosition
├──────────────────────────────────┤
│                                  │
│    AbsolutePosition.Y = 104     │  ← Button ada di sini (viewport-relative)
│    Screen Y seharusnya = 162     │  ← VIM perlu koordinat ini (screen-absolute)
│                                  │
└──────────────────────────────────┘
```

Tanpa koreksi, semua klik mendarat **58px di atas tombol** yang sebenarnya.

## Solusi

### 1. GuiService Inset Correction (Fix Utama)

Menambahkan fungsi `getGuiInsetTop()` dan `guiToScreenCoords()` yang mengoreksi koordinat:
...
Semua tempat yang sebelumnya pakai `AbsolutePosition` langsung untuk VIM/executor mouse sekarang pakai `guiToScreenCoords()`.

### 2. Method Baru Ditambahkan

| Method | Deskripsi |
|--------|-----------|
| `fireclick` | Menggunakan executor `fireclick()` function jika tersedia |
| `fire_remote_from_connections` | Scan connections button, panggil `conn.Function()` langsung + cari RemoteEvent di children |

### 3. Diagnostic Logging

Log startup sekarang menampilkan:
- `GuiService inset Y` — nilai offset yang dikoreksi
- `screenXY` — koordinat screen setelah koreksi
- Availability `fireclick`, `getconnections`, `firesignal`

## Hasil

```
[10:50:29] GuiService inset Y: 58px (coordinate correction active)
[10:50:29] Ready detail: ... pos=(1058,104) ... screenXY=(1058,162) | insetY=58
[10:50:30] Ready clicked from WaveInfo via executor_mouse_click (count=1)
[10:50:38] RapidFire Ready #1 ... prog=23/24 reason=button_hidden:23
```

| Sebelum Fix | Sesudah Fix |
|-------------|-------------|
| `pos=(1058,104)` → klik di Y=104 | `screenXY=(1058,162)` → klik di Y=162 |
| `prog=0/18` semua NOPROG | `prog=23/24` button_hidden (sukses!) |
| 3x RapidFire gagal → timeout | 1x click langsung berhasil |

## Metode yang Berhasil

> **`executor_mouse_click`** — menggunakan `mousemoveabs()` + `mouse1click()` dengan koordinat yang sudah dikoreksi oleh `guiToScreenCoords()`.

Ini masuk akal karena executor user memiliki:
- `mousemoveabs` ✅
- `mouse1click` ✅
- `fireclick` ❌ (tidak tersedia)
- `firesignal` ❌ (tidak tersedia)

Dengan koreksi inset **58px**, `executor_mouse_click` langsung berhasil pada percobaan pertama.

## Files Modified

```diff:SummonHero_AutoFarm_Entries.lua
--[[
    ==========================================================
...
local function doSweepVirtualClick(target)
    ...
    for _, p in ipairs(points) do
        ...
        local cx = math.floor(pos.X + (size.X * rx))
        local cy = math.floor(pos.Y + (size.Y * ry))
...
local function handleReadyState()
    ...
    throttledLog("ready_target_detail", 4, "Ready detail: path=" .. targetPath .. " | " .. guiRectText(readyBtn) .. " | active=" .. tostring(readyBtn.Active))
